    ...
    writeOgcExpression: function(value, node) {
        if (value instanceof OpenLayers.Filter.Function){
            ...
            if(typeof value == "string"){
                //split out ${propname} and write them as <ValueReference> nodes
                if(/\${[^${}]*}/.test(value)){
                    var valueExps = value.split(/\${([^${}]+)/);
                    for(var i=0;i<valueExps.length;i++){
                        if(/[^${}]+/.test(valueExps[i])){
                            this.writeNode("ValueReference", {property: valueExps[i]}, node);
                        }
                    }
                    return node;
                }
